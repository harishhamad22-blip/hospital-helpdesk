...
// ║   IT Hardware Helpdesk System — v3.0 (Zero-Failure Edition)  ║
// ╚══════════════════════════════════════════════════════════════╝

// ── WHATSAPP CONFIG ───────────────────────────────────────────
// Option A: CallMeBot (Free WhatsApp API) — Register at callmebot.com
// Option B: WhatsApp Group invite link for manual redirect
// IMPORTANT: Replace WHATSAPP_GROUP_LINK with your actual group invite link
...
const sendWhatsAppAlert = async (complaint) => {
  // Method 1: CallMeBot API (if configured)
  if (CALLMEBOT_PHONE && CALLMEBOT_API_KEY) {
    const msg = encodeURIComponent(
      `🚨 *New IT Complaint - CHRC*\n\n` +
      `*Ticket:* ${complaint.id}\n` +
      `*User:* ${complaint.userName}\n` +
      `*Dept:* ${complaint.dept}\n` +
      `*Type:* ${complaint.type}${complaint.printerType ? ` (${complaint.printerType})` : ''}\n` +
  ...
  // Method 2: Open WhatsApp group link in new tab as fallback notification
  // (Only admin sees this popup, not the user)
};
...
const FireDB = {
  ...
  async clearBroadcast() {
    try { await setDoc(doc(db, 'meta', 'broadcast'), { active: false, message: '', by: '', at: '' }); }
    catch (e) { console.error(e); }
  },
...
// ── THEME ─────────────────────────────────────────────────────
const C = {
  navy: '#0a1628', navy2: '#0d1f3c', navy3: '#122244',
  gold: '#c9a84c', gold2: '#e8c96c', goldL: '#fdf6e3',
  white: '#ffffff', off: '#f7f8fc', card: '#ffffff',
  border: '#e4e9f2', border2: '#c8d0e0',
  text: '#0d1f3c', text2: '#2d3d5c', muted: '#6b7a99',
...
function Card({ children, style = {}, className = '' }) {
  return (
    <div className={className} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 16, boxShadow: '0 2px 12px #0a162810', ...style }}>
      {children}
    </div>
  );
}
...
function LoginPage({ onLogin }) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // FIX: safe comparison using safeLC
  ...
  const handleForgotPassword = async () => {
    ...
    if (fpNew1.length < 6) { setFpError('Password must be at least 6 characters'); return; }
    if (fpNew1 !== fpNew2) { setFpError('Passwords do not match'); return; }
    if (fpNew1 === 'Chrc@1234') { setFpError('Please choose a different password'); return; }
    setFpLoading(true);
    try {
      const users = await FireDB.getUsers();
          ...
          {tab === 'login' ? (
            <>
              <div style={{ marginBottom: 24, paddingBottom: 20, borderBottom: `1px solid ${C.border}` }}>
                <h2 style={{ fontWeight: 700, fontSize: 20, color: C.text, fontFamily: "'Cormorant Garamond',serif" }}>Welcome Back</h2>
            ...
            <>
              <div style={{ marginBottom: 24, paddingBottom: 20, borderBottom: `1px solid ${C.border}` }}>
                <h2 style={{ fontWeight: 700, fontSize: 20, color: C.text, fontFamily: "'Cormorant Garamond',serif" }}>Reset Password</h2>
                <p style={{ color: C.muted, fontSize: 13, marginTop: 4 }}>Enter your username and set a new password</p>
              </div>
              <div style={{ marginBottom: 18 }}>
    ...
    <div style={{
      ...
      <div className="fadeUp" style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          <h2 style={{ color: '#fff', fontSize: 22, fontWeight: 700, fontFamily: "'Cormorant Garamond',serif" }}>Set Your Password</h2>
          <p style={{ color: 'rgba(255,255,255,0.65)', fontSize: 13, marginTop: 8, lineHeight: 1.6 }}>
            Welcome, <strong style={{ color: '#fff' }}>{user.displayName}</strong>!<br />Create a secure password to continue.
          </p>
        </div>
        <div style={{ background: 'rgba(255,255,255,0.97)', borderRadius: 24, padding: 30, boxShadow: '0 30px 80px rgba(0,0,0,0.4)' }}>
          <div style={{ marginBottom: 18 }}>
  ...
  const submit = async () => {
    ...
    const ticket = {
      ...
      history: [{
        ...
        note: `Complaint submitted by ${user.displayName}. IT team will respond soon. 🙏`
      }],
      actionBy: '', solution: '', actionAt: '', holdReason: '', refuseReason: ''
    };
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 62 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <Btn onClick={onLogout} variant="ghost" size="sm"
                style={{ color: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.2)' }}>Logout</Btn>
            </div>
          </div>
              ...
              <div style={{
                ...
                <div>
                  ...
                  <div style={{ fontSize: 12, color: C.green, marginTop: 6, fontWeight: 600 }}>
                    🙏 IT team will respond soon. You can track status in "My Complaints" tab.
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }} className="grid-2">
              {[
                ...
                ...(selected.status === 'resolved' || selected.status === 'closed' ? [
                  ['Action By (IT)', selected.actionBy || '—', C.green, false],
                  ['Resolved At', fmtDT(selected.actionAt) || '—', C.green, false],
                ] : [])
              ].map(([k, v, c, mono]) => (
...
function AdminPortal({ user, onLogout }) {
  ...
  const stats = useMemo(() => {
    return {
      ...
      hold: complaints.filter(c => c.status === 'hold').length,
      resolved: complaints.filter(c => c.status === 'resolved' || c.status === 'closed').length,
      refused: complaints.filter(c => c.status === 'refused').length,
    };
  }, [complaints]);
  ...
  const doAction = async () => {
    ...
    let newStatus = '', histEntry = {}, updateData = {};
  ...
  const handleAddUser = async () => {
    ...
    if (!newUser.password.trim() || newUser.password.length < 6) { setAddUserError('Password must be at least 6 characters'); return; }
  ...
  const exportCSV = () => {
    const hdr = ['Ticket ID', 'User Name', 'Emp ID', 'Department', 'Type', 'Sub Type', 'Description', 'Status', 'Submitted', 'Action By', 'Solution', 'Hold Reason', 'Refused Reason'];
    const rows = filtered.map(c => [
      c.id, c.userName, c.empId, c.dept, c.type, c.printerType || '',
      `"${(c.desc || '').replace(/"/g, '""')}"`, c.status,
      fmtDT(c.at), c.actionBy || '', c.solution || '', c.holdReason || '', c.refuseReason || ''
    ]);
  ...
  const getActionButtons = (c, inModal = false) => {
    ...
    if (c.status === 'open' || c.status === 'hold') {
      return (
        <>
          ...
          {c.status === 'open' && <Btn onClick={hold} variant="warning" size="sm">⏸️ Hold</Btn>}
    ...
    if (c.status === 'resolved') {
      return <Btn onClick={close} variant="ghost" size="sm">🔒 Close</Btn>;
    }
    return null;
  };
    ...
    <div style={{ minHeight: '100vh', background: C.off }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', height: 65 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              ...
              <Btn onClick={() => setPwModal(true)} variant="ghost" size="sm"
                style={{ color: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.2)', fontSize: 12 }}>🔑</Btn>
              <Btn onClick={onLogout} variant="ghost" size="sm"
                style={{ color: 'rgba(255,255,255,0.7)', border: '1px solid rgba(255,255,255,0.2)', fontSize: 12 }}>Logout</Btn>
            </div>
          </div>
      ...
      <div style={{ maxWidth: 1400, margin: '0 auto', padding: '24px 20px' }}>
        ...
        <div style={{ display: 'flex', gap: 14, marginBottom: 24, flexWrap: 'wrap' }}>
          ...
          <StatCard icon="🔵" label="Open" value={stats.open} color='#1e40af' bg='#dbeafe' />
          <StatCard icon="⏸️" label="On Hold" value={stats.hold} color={C.yellow} bg={C.yellowL} />
          <StatCard icon="✅" label="Resolved/Closed" value={stats.resolved} color={C.green} bg={C.greenL} />
                  ...
                  <Card key={c._docId || c.id} style={{ padding: 20, transition: 'all .2s' }}
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 12, flexWrap: 'wrap', gap: 10 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 15, fontWeight: 700, color: C.text, fontFamily: "'Cormorant Garamond',serif" }}>
          ...
          <div className="fadeUp">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 18 }} className="grid-2">
              <Card style={{ padding: 22 }}>
                <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 18, color: C.text, fontFamily: "'Cormorant Garamond',serif" }}>Monthly Trend</div>
                {chartData.length === 0 ? (
              ...
              <Card style={{ padding: 22 }}>
                <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 18, color: C.text, fontFamily: "'Cormorant Garamond',serif" }}>By Type</div>
                {typeData.length === 0 ? (
                  <div style={{ textAlign: 'center', color: C.muted, padding: 40 }}>No data yet</div>
                ) : (
                  <ResponsiveContainer width="100%" height={260}>
            ...
            <Card style={{ padding: 22 }}>
              <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 18, color: C.text, fontFamily: "'Cormorant Garamond',serif" }}>Monthly Status Breakdown</div>
              {chartData.length === 0 ? (
                <div style={{ textAlign: 'center', color: C.muted, padding: 40 }}>No data yet</div>
              ) : (
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={chartData} margin={{ top: 5, right: 5, bottom: 5, left: -20 }}>
                    ...
                    <Bar dataKey="hold" fill="#d97706" name="On Hold" radius={[4, 4, 0, 0]} />
          ...
          <div className="fadeUp">
            <Card style={{ padding: 28, marginBottom: 18 }}>
              ...
              {broadcast?.active && broadcast?.message ? (
                <div style={{ background: 'linear-gradient(135deg,#f3e8ff,#ede9fe)', borderRadius: 14, padding: 20, marginBottom: 20, border: `1px solid #c4b5fd` }}>
                  <div style={{ fontWeight: 700, color: '#5b21b6', fontSize: 12, letterSpacing: .6, textTransform: 'uppercase', marginBottom: 8 }}>📡 Active Message</div>
                  <div style={{ fontSize: 15, color: '#3b0764', lineHeight: 1.6, fontWeight: 500 }}>{broadcast.message}</div>
                  <div style={{ fontSize: 11, color: '#7c3aed', marginTop: 8 }}>By {broadcast.by} · {fmtDT(broadcast.at)}</div>
                  <Btn onClick={() => FireDB.clearBroadcast()} variant="danger" size="sm" style={{ marginTop: 14 }}>🗑️ Delete</Btn>
                </div>
              ) : (
                <div style={{ background: C.off, borderRadius: 14, padding: 20, marginBottom: 20, border: `1px solid ${C.border}`, textAlign: 'center' }}>
                  <div style={{ fontSize: 32, marginBottom: 8 }}>🔕</div>
                  <div style={{ color: C.muted, fontSize: 14, fontWeight: 500 }}>No active broadcast</div>
                </div>
              )}
              ...
              <Btn onClick={sendBroadcast} variant="purple" size="md">📢 Send Broadcast</Btn>
            </Card>
            ...
            <Card style={{ padding: 24 }}>
              ...
              {CALLMEBOT_PHONE && CALLMEBOT_API_KEY ? (
                <div style={{ background: '#f0fdf4', borderRadius: 12, padding: 16, border: `1px solid #a7f3d0` }}>
                  <div style={{ color: C.green, fontWeight: 700, fontSize: 14 }}>✅ WhatsApp notifications are active!</div>
                  <div style={{ color: C.green, fontSize: 13, marginTop: 4 }}>Number: +{CALLMEBOT_PHONE}</div>
                </div>
              ) : (
                <div style={{ background: C.yellowL, borderRadius: 12, padding: 16, border: `1px solid #fde68a` }}>
                  <div style={{ color: C.yellow, fontWeight: 700, fontSize: 14, marginBottom: 8 }}>⚠️ Setup Required</div>
                  <div style={{ color: C.text2, fontSize: 13, lineHeight: 1.8 }}>
                    To enable WhatsApp alerts when new complaints are submitted:<br />
                    1. Go to <strong>callmebot.com</strong> and register your WhatsApp number<br />
                    2. Get your API key from CallMeBot<br />
                    3. In <code>App.js</code>, update <code>CALLMEBOT_PHONE</code> and <code>CALLMEBOT_API_KEY</code><br />
                    4. Also update <code>WHATSAPP_GROUP_LINK</code> with your group invite link
                  </div>
                  {WHATSAPP_GROUP_LINK !== 'https://chat.whatsapp.com/YOUR_GROUP_INVITE_CODE' && (
                    <a href={WHATSAPP_GROUP_LINK} target="_blank" rel="noreferrer"
                      style={{
        ...
        {tab === 'users' && (
          <div className="fadeUp">
            <Card style={{ padding: 20, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14, flexWrap: 'wrap', gap: 10 }}>
                <div style={{ fontWeight: 700, fontSize: 16, color: C.text, fontFamily: "'Cormorant Garamond',serif" }}>User Management</div>
                <div style={{ display: 'flex', gap: 10 }}>
                  <span style={{ fontSize: 12, color: C.muted, fontWeight: 600, alignSelf: 'center' }}>{filteredUsers.length} users</span>
                  <Btn onClick={() => { setAddUserModal(true); setAddUserError(''); setNewUser({ username: '', displayName: '', password: '', role: 'user' }); }}
          ...
          <div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }} className="grid-2">
              {[
                ...
                ...(detailModal.status === 'resolved' || detailModal.status === 'closed' ? [
                  ['Action By', detailModal.actionBy || '—', C.green, false],
            ...
            {detailModal.holdReason && detailModal.status === 'hold' && (
              <div style={{ background: C.yellowL, borderRadius: 10, padding: '12px 14px', marginBottom: 18, border: `1px solid #fde68a` }}>
                <div style={{ fontSize: 10, color: C.yellow, fontWeight: 700, marginBottom: 6, textTransform: 'uppercase' }}>Hold Reason</div>
      ...
      <Modal open={!!actionModal} onClose={() => setActionModal(null)}
        title={actionType === 'resolve' ? '✅ Resolve Complaint' : actionType === 'hold' ? '⏸️ Put on Hold' : actionType === 'refuse' ? '❌ Refuse Complaint' : '🔒 Close Complaint'}
        width={480}>
        {actionModal && (
          <div>
            ...
            {actionType === 'hold' && (
              <>
                <div style={{ marginBottom: 18 }}>
                  <FieldLabel required>Reason for Hold</FieldLabel>
                  <textarea value={actionForm.reason} onChange={e => af('reason', e.target.value)}
                    rows={3} placeholder="Why is this complaint being put on hold? (e.g., waiting for parts, vendor support needed...)"
                    style={{ ...inputStyle, resize: 'vertical' }} />
                </div>
                <Btn onClick={doAction} variant="warning" size="md" style={{ width: '100%' }}>⏸️ Put on Hold</Btn>
              </>
            )}
            {actionType === 'refuse' && (
              <>
                <div style={{ marginBottom: 18 }}>
                  ...
                  <textarea value={actionForm.reason} onChange={e => af('reason', e.target.value)}
                    rows={3} placeholder="Why is this complaint being refused?"
                    style={{ ...inputStyle, resize: 'vertical' }} />
                </div>
                <Btn onClick={doAction} variant="danger" size="md" style={{ width: '100%' }}>❌ Refuse Complaint</Btn>
              </>
            )}
            {actionType === 'close' && (
              <>
      ...
      {/* Add User Modal */}
      <Modal open={addUserModal} onClose={() => setAddUserModal(false)} title="➕ Add New User" width={440}>
        <div>
          <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ marginBottom: 18 }}>
            ...
            <select value={newUser.role} onChange={e => nu('role', e.target.value)} style={inputStyle}>
              <option value="user">User (Employee)</option>
              <option value="admin">Admin (IT Staff)</option>
            </select>
          </div>
          {addUserError && (
            <div style={{
